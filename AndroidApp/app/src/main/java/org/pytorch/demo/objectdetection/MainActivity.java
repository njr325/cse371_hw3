...
public class MainActivity extends AppCompatActivity implements Runnable {
    private String URL = "http://172.20.10.2:8008/predict"; // Change this address to the address of your cloud server
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            mModule = LiteModuleLoader.load(MainActivity.assetFilePath(getApplicationContext(), "yolov5m.torchscript.ptl"));
    ...
    public void run() {
        if (mRadioMobile.isChecked()){
            ...
            runOnUiThread(() -> {
                ...
                mTimeTextView.setText("Time taken(ms): " + inferenceTime);
            });
        }
        else {
            ...
            StringRequest request = new StringRequest(Request.Method.POST, URL, new Response.Listener<String>() {
                ...
                public void onResponse(String response) { // Callback function that handles the response
                    ...
                    try {
                        ...
                        for (int i = 0; i < detections.length(); i++) {
                            ...
                            float x1 = (float) det.getDouble("x1");
                            float y1 = (float) det.getDouble("y1");
                            float x2 = (float) det.getDouble("x2");
                            float y2 = (float) det.getDouble("y2");
                            float score = (float) det.getDouble("score");
                            String clsName = det.getString("cls");

                            // Map class name to index
                            int clsIdx = 0;
                            for (int j = 0; j < PrePostProcessor.mClasses.length; j++) {
                                if (PrePostProcessor.mClasses[j].equals(clsName)) {
                                    clsIdx = j;
                                    break;
                                }
                            }

                            results.add(boxToResult(x1, y1, x2, y2, clsIdx, score));
                        }

                    } catch (JSONException e) {
                        Log.e("Cloud Inference", "JSON parse error: " + e);
                    }
                ...
                public void onErrorResponse(VolleyError volleyError) {
                    Log.e("Cloud Inference", "Some error occurred -> " + volleyError.toString());
                    runOnUiThread(() -> {
                        Toast.makeText(MainActivity.this, "Cloud inference failed", Toast.LENGTH_SHORT).show();
                        mButtonDetect.setEnabled(true);
                        mProgressBar.setVisibility(ProgressBar.INVISIBLE);
                    });
                }
                ...
                protected Map<String, String> getParams() throws AuthFailureError { // Function that 
                    ...
                    parameters.put("data", encodedImage);
